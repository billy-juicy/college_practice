...
class MainWindow:
    def __init__(self, master, user):
        ...
        role = user[2]
        self.permissions = ROLE_PERMISSIONS.get(role, ROLE_PERMISSIONS["partner"])
        ...
        # --- Приветствие ---
        tk.Label(master, text=f"Добро пожаловать, {user[1]} ({role})",
        ...
        # Кнопки, зависящие от роли
        if self.permissions["can_manage_partners"]:
        ...
        if self.permissions["can_manage_materials"]:
        ...
        if self.permissions["can_manage_services"]:
        ...
        if self.permissions["can_manage_orders"]:
    ...
    def add_partner_card(self, partner):
        ...
        for label, value in zip(["ID", "Наименование", "Тип", "Рейтинг", "Юридический адрес", "ИНН",
                                 "ФИО руководителя", "Телефон", "Email", "Логотип"], partner):
            lbl = tk.Label(card, text=f"{label}: {value}", font=FONT_SMALL, bg="white", anchor="w")
            lbl.pack(fill="x")
    ...
    def delete_selected_partner(self):
        if not self.selected_card:
            messagebox.showwarning("Ошибка", "Выберите партнёра")
            return
        if not messagebox.askyesno("Подтверждение", "Удалить партнёра?"):
            return
        ...
        try:
            ...
            conn.commit()
            messagebox.showinfo("Успех", "Партнёр удалён")
            self.load_partners()
        finally:
            conn.close()
